...
func getIpcShmHandler(packet tlm.TelemetryPacket, write bool, shmDir string) (*ipc.IpcShmHandler, error) {
	handler, err := ipc.CreateIpcShmHandler(strconv.Itoa(packet.Port), GetPacketSize(packet), write, shmDir)
	if err != nil {
